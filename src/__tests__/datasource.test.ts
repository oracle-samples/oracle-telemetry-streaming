...
it("applyAdhocFilters applies label filters", () => {
  ...
  const result = (ds as any).applyAdhocFilters("metric", [
    { key: "job", operator: "=", value: "node" },
  ]);
...
it("getTagKeys returns metric keys", async () => {
  ...
  jest
    .spyOn(ds, "query")
    .mockReturnValue(
      mockQueryResponse(JSON.stringify({ data: ["job", "instance"] })) as any
    );
...
it("getTagValues returns values for key", async () => {
  ...
  jest
    .spyOn(ds, "query")
    .mockReturnValue(
      mockQueryResponse(JSON.stringify({ data: ["us-east", "us-west"] })) as any
    );
        ...
        fields: [
          {
            values: {
              buffer: [
                JSON.stringify({
                  data: [
                    { __name__: "up", job: "node", instance: "localhost" },
                  ],
                }),
              ],
            },
...
it("fetchStaticLabels returns label list", async () => {
  ...
  jest
    .spyOn(ds, "query")
    .mockReturnValue(
      mockQueryResponse(JSON.stringify({ data: ["cpu", "memory"] })) as any
    );
